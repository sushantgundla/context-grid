...
| `one_chunk_per_document` | chunk | CAUTION | The chunk size is larger than the documents, so every document came out whole and retrieval ranked documents rather than passages. The chunker axis cannot move a number it never touched, and configurations that tie here have not been compared — they have not been tried. Sweep smaller sizes, or measure on longer documents. `contextgrid profile` says the same thing from the corpus alone. |
