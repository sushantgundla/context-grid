...
```
markdown · fixed:20,overlap=0 · tfidf · dense         0.889  0.833
markdown · sentence:1 · tfidf · dense                 0.778  0.833

winner.has('weighted_recall@5'): True
winner.has('made_up_metric@5'): False
markdown · fixed:20,overlap=0 · tfidf · dense scored best on weighted_recall@5 at 0.889, across 2 configurations, scored on 3 questions. markdown · fixed:20,overlap=0 · tfidf · dense and markdown · sentence:1 · tfidf · dense are not distinguishable on this eval set (n=3). The gap of +0.111 on weighted_recall@5 sits inside the confidence interval +0.000 to +0.333, so it is consistent with no difference at all. Settling a gap this size would take roughly 320 questions -- on a two-sided test at alpha 0.05 with 80% power, assuming per-question scores vary as much as a 0-1 score possibly can. It is an order of magnitude, not a count. It runs locally at no cost per query, answering at under 1 ms p95. 1 of 3 questions failed. 100% of those are fp7_incomplete: part of the evidence was retrieved and part was not. The answer needs passages that no single chunk holds, so try parent-document retrieval or a larger k. This was a retrieval-only run, so failure points four to seven -- the ones about what the generator did with the context -- cannot be seen from here.
```
