...
| `chunk` | `chonkie>=1.7`, `langchain-text-splitters>=0.3`, `litellm>=1.60` | Light-medium. `chonkie` ~2.1 MB, `langchain-text-splitters` ~300 KB. `litellm` is here too (see below) at ~114 MB installed — it bundles a large price table and its own dependency tree. | Chunkers `chonkie:recursive`, `chonkie:sentence`, `chonkie:token`, `chonkie:code`, `langchain:recursive`, `langchain:character`, `langchain:markdown` |
...
| `judge` | `deepeval>=4.0` | Heavy — ~13 MB for the package itself, plus its own sizeable dependency tree for running generation metrics. | Faithfulness, answer-relevancy and the rest of the generation metrics, run through whichever model `run.model` already points at |
