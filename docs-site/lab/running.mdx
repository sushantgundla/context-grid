...
A `MemoryCache` dies with the process. Pass a `cg.DiskCache` instead and the same sweep,
re-run tomorrow from a fresh interpreter, is a hit end to end:
...
The 80% is prefix sharing inside the first sweep. The 100% is the second process reading what
the first one wrote. Two of these can also run at the same time — see
[Caching](/reference/caching) for what is and is not safe when they share a directory.

<Note>
Earlier versions dropped the `DiskCache` you passed and used a `MemoryCache` instead, because
an empty `DiskCache` is falsy and `Lab.__init__` chose with `cache or MemoryCache()`. Every
first run starts empty, so nothing was ever written and the directory never stopped being
empty. If you are on a version where `len(cache)` stays `0` after a sweep, that is this bug and
...
`BUDGET_REACHED` means exactly one thing — this sweep stopped, or could not start. Two other
facts used to share the code and no longer do, so the filter above catches neither:
`MODEL_NOT_PRICED` (a model with no published price, costed at zero) and `NO_COST_CEILING` (a
model-calling plugin running with no budget at all, and `budget_usd` not covering machine time).

You do not have to read warnings to find out. `results.is_partial` is the flag, `results.planned`
is how many configurations were meant to run, `results.stopped` is why it stopped, and
`results.partial_note()` is the sentence — which `contextgrid run` prints on **stdout** above the
leaderboard, and `build_manifest(..., notes=results.manifest_note())` writes into a bundle:
...
<Note>
`budget_usd` counts **token spend only**. Machine time — `compute_seconds` priced through
`machine_usd_per_hour` — is measured and reported per configuration on `RunResult.cost.machine_usd`,
and is deliberately not charged against the budget: it falls whenever a cache is warm, so a limit
counting it stopped after two configurations on one run and four on the next out of the same
money. Use `budget_seconds` to bound wall-clock. A sweep that sets both a machine rate and
`budget_usd` gets a `NO_COST_CEILING` warning saying so.
</Note>
...
**A configuration that cannot be built is one lost row, not a lost sweep.** Give `grid()` a
chunker, embedder, index or reranker spec that does not exist, and that configuration is left
out of the leaderboard with a `configuration_failed` warning saying so. Every other
...
The same containment covers a plugin that genuinely cannot run in this environment — a missing
extra, for instance. `contextgrid run` prints the reason on **stdout** as well as stderr when
the failure leaves nothing measured at all, and exits `1`, so a matrix whose every cell needs
an uninstalled extra cannot come back green with an empty leaderboard.

<Note>
  Absent, not zero. A failed configuration has no row rather than a row of `0.000`, because a
  zero is a measurement and nothing here measured anything. The same rule governs a metric no
  run computed: `leaderboard()` omits the key, and both the terminal table and `report.md`
  print `NOT_MEASURED` in that cell.
</Note>

Impossible *combinations* of otherwise-valid values (a dense index with no embedder) are the
exception in the other direction: those are dropped before the sweep starts, not discovered by
trying to build them — see [Defining a Sweep](/lab/grid).
