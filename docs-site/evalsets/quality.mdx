...
CAUTION [evalset]: 3 questions carry evidence, unchecked against a corpus, so this set can only detect differences of about 1.00 or larger. Anything smaller than that on a leaderboard built from this set is noise
...
| `.can_support(difference) -> bool` | can this set reliably see a gap of this size — tested against the exact `.detectable_difference` |
...
<Note>
The printed floor is rounded **up**, not to nearest, so that the sentence is true as written
and the number can be handed straight back. On 24 questions the exact floor is `0.404145`:
`.summary()` says `detects differences of 0.41 and above`, and `can_support(0.41)` is `True`.
It used to print `0.40` — the number the same object then rejected.
</Note>

`minimum_detectable_difference(n, *, power=0.8, alpha=0.05)` is an approximate two-sided
sample-size formula, worst-case variance. It uses two-value lookup tables for the
significance constants, not a real inverse-normal computation — passing e.g. `alpha=0.10`
doesn't smoothly move the answer, it just switches to the other bucket's constant. `n < 2`
returns `1.0` rather than erroring. It isn't `cg.X`; import it as
