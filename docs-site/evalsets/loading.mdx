...
`meta` is written as JSON in a single cell — ugly in a spreadsheet, but it round-trips, and
`meta.reviewed` is what `assess()` counts for its "% reviewed" figure. (JSON has no tuples:
a `("a", 1)` inside `meta` comes back as `["a", 1]`.)

<Warning>
Two things still don't fit the format, and `write_csv` now raises a `UserWarning` naming the
questions affected rather than dropping them silently:

- Only `item.anchors[0]` — the first anchor — is written. An item with more than one anchor
  loses the rest on the way to CSV.
- `gold` (span-form evidence) has no column at all. The anchors are written, so re-resolving
  against a parse gets the spans back; an item carrying *only* `gold` loses its evidence.

Use `cg.write_jsonl` for anything that has to survive intact.
</Warning>
