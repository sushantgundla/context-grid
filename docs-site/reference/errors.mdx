...
The message is built as `f"{feature} requires the '{extra}' extra{needs}. Install it with: {hint}"`
...
`feature` is the subject of that sentence, so it is always a short noun phrase — `The 'docling'
parser`, `The faiss index`. A few sites have more to say than that; they pass an optional
