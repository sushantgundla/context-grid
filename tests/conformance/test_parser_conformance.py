...
from tests.support import ALL_PARSERS, SAMPLE_SOURCES, source
...
@pytest.fixture(params=SAMPLE_SOURCES, ids=SOURCE_IDS)
def sample(request: pytest.FixtureRequest) -> SourceFile:
    return request.param  # type: ignore[no-any-return]
...
def test_empty_document_parses_without_raising(parser: Parser) -> None:
    parsed = parser.parse(source("empty", ""))
